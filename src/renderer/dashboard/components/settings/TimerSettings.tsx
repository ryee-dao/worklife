...
export default function TimerSettings() {
  ...
  return (
    <div className="p-3 flex flex-col h-full">
      <h2 className="font-semibold text-2xl">Timer Settings</h2>
      ...
      <div className="self-center text-slate-800 tracking-wider font-medium flex flex-col items-center">
        <button
          ...
          className={`px-3 py-0.5 border rounded bg-blue-300 w-48 ${
            !isValid ? "cursor-not-allowed bg-slate-400" : "active:bg-blue-500"
          }`}
        >
          SAVE CHANGES
        </button>
        {saveMessage && (
          <p className={`text-sm ${saveStatus === 'error' ?'text-red-600': 'text-green-600'} mt-1`}>{saveMessage}</p>
        )}
      </div>
    </div>
  );
